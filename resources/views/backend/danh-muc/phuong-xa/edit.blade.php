                ...
                <div class="col-md-12">
                    <div class="card card-primary">
                        <div class="card-header">
                            <h3 class="card-title">Sửa phường xã</h3>
                        </div>

                        <form action="{{ route('admin.phuong_xa.update', $phuongXa->id) }}" method="POST">
                            @csrf
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('quan_huyen_id') is-invalid @enderror" name="quan_huyen_id">
                                        ...
                                        @foreach($quanHuyens as $item)
                                            <option  @if($phuongXa->quan_huyen_id == $item->id) selected @endif value="{{ $item->id }}">{{ $item->name }}</option>
                                        @endforeach
                                    </select>
